 ...
 */
Ext.define('iBOSe.view.reminder.src.view.DayBody', {
    extend: 'iBOSe.view.reminder.src.view.AbstractCalendar',
    alias: 'widget.daybodyview',

    requires: [
        'Ext.XTemplate'
        //'iBOSe.view.reminder.src.template.DayBody',
        //'iBOSe.view.reminder.src.data.EventMappings',
        //'iBOSe.view.reminder.src.dd.DayDragZone',
        //'iBOSe.view.reminder.src.dd.DayDropZone'
    ],
    ...
    initDD: function() {
        ...
        this.dragZone = new iBOSe.view.reminder.src.dd.DayDragZone(this.el, Ext.apply({
            view: this,
            containerScroll: true
        },
        cfg));

        this.dropZone = new iBOSe.view.reminder.src.dd.DayDropZone(this.el, Ext.apply({
            view: this
        },
        cfg));
    },
    ...
    afterRender: function() {
        if (!this.tpl) {
            this.tpl = new iBOSe.view.reminder.src.template.DayBody({
                id: this.id,
    ...
    getTemplateEventData: function(evt) {
        var selector = this.getEventSelectorCls(evt[iBOSe.view.reminder.src.data.EventMappings.EventId.name]),
            data = {},
        ...
        for (; day < this.dayCount; day++) {
            ...
            for (; ev < ct; ev++) {
                ...
                evts.push({
                    ...
                    date: iBOSe.view.reminder.src.util.Date.add(this.viewStart, {days: day})
                });
            }
        }
    ...
    getDayAt: function(x, y) {
        var xoffset = this.el.down('.ext-cal-day-times').getWidth(),
            ...
            mins = rowIndex * 30,
            dt = iBOSe.view.reminder.src.util.Date.add(this.viewStart, {days: dayIndex, minutes: mins}),
            el = this.getDayEl(dt),
            timeX = x;
    ...
    onClick: function(e, t) {
        if (this.dragPending || iBOSe.view.reminder.src.view.DayBody.superclass.onClick.apply(this, arguments)) {
