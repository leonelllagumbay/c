 ...
 */
Ext.define('iBOSe.view.reminder.src.view.AbstractCalendar', {
    ...
    getWeekCount: function() {
        var days = iBOSe.view.reminder.src.util.Date.diffDays(this.viewStart, this.viewEnd);
    ...
    prepareData: function() {
        ...
        for (; w < weeks; w++) {
            ...
            for (d = 0; d < this.dayCount; d++) {
                ...
                dt = iBOSe.view.reminder.src.util.Date.add(dt, {days: 1});
            }
        }
    ...
    onCalendarEndDrag: function(start, end, onComplete) {
        if (start && end) {
            ...
            var o = {};
            o[iBOSe.view.reminder.src.data.EventMappings.StartDate.name] = start;
            o[iBOSe.view.reminder.src.data.EventMappings.EndDate.name] = end;
    ...
    onUpdate: function(ds, rec, operation) {
        ...
        if (operation == Ext.data.Record.COMMIT) {
            ...
            if (this.enableFx && this.enableUpdateFx) {
                this.doUpdateFx(this.getEventEls(rec.data[iBOSe.view.reminder.src.data.EventMappings.EventId.name]), {
                    scope: this
                });
            }
        }
    ...
    onAdd: function(ds, records, index) {
        ...
        if (this.enableFx && this.enableAddFx) {
            this.doAddFx(this.getEventEls(rec.data[iBOSe.view.reminder.src.data.EventMappings.EventId.name]), {
                scope: this
            });
        }
    },
    ...
    onRemove: function(ds, recs) {
        var name = iBOSe.view.reminder.src.data.EventMappings.EventId.name,
            i, len, rec, els;
    ...
    setViewBounds: function(startDate) {
        var start = startDate || this.startDate,
            ...
            Dt = iBOSe.view.reminder.src.util.Date;
    ...
    moveNext: function(noRefresh) {
        return this.moveTo(iBOSe.view.reminder.src.util.Date.add(this.viewEnd, {days: 1}));
    },
    ...
    movePrev: function(noRefresh) {
        var days = iBOSe.view.reminder.src.util.Date.diffDays(this.viewStart, this.viewEnd) + 1;
    ...
    moveMonths: function(value, noRefresh) {
        return this.moveTo(iBOSe.view.reminder.src.util.Date.add(this.startDate, {months: value}), noRefresh);
    },
    ...
    moveWeeks: function(value, noRefresh) {
        return this.moveTo(iBOSe.view.reminder.src.util.Date.add(this.startDate, {days: value * 7}), noRefresh);
    },
    ...
    moveDays: function(value, noRefresh) {
        return this.moveTo(iBOSe.view.reminder.src.util.Date.add(this.startDate, {days: value}), noRefresh);
    },
    ...
    getEventRecord: function(id) {
        var idx = this.store.find(iBOSe.view.reminder.src.data.EventMappings.EventId.name, id);
