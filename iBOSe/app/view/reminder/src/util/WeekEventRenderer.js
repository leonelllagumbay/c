 ...
 */
Ext.define('iBOSe.view.reminder.src.util.WeekEventRenderer', {

    //requires: ['iBOSe.view.reminder.src.util.Date'],
        ...
        render: function(o) {
            ...
            for (; w < weekCount; w++) {
                if (!grid[w] || grid[w].length == 0) {
                    ...
                    dt = iBOSe.view.reminder.src.util.Date.add(dt, {days: 7});
                } else {
                    var row,
                        d = 0,
                        wk = grid[w],
                        startOfWeek = Ext.Date.clone(dt),
                        endOfWeek = iBOSe.view.reminder.src.util.Date.add(startOfWeek, {days: dayCount, millis: -1});
