 ...
 */
Ext.define('iBOSe.view.reminder.src.form.EventDetails', {
    extend: 'Ext.form.Panel',
    alias: 'widget.eventeditform',
    
    requires: [
        //'iBOSe.view.reminder.src.form.field.DateRange',
        //'iBOSe.view.reminder.src.form.field.ReminderCombo',
        //'iBOSe.view.reminder.src.data.EventMappings',
        //'iBOSe.view.reminder.src.form.field.CalendarCombo'
    ],
    ...
    initComponent: function() {
        this.titleField = new Ext.form.Text({
            fieldLabel: 'Title',
            name: iBOSe.view.reminder.src.data.EventMappings.Title.name,
            emptyText: 'Event Title',
            allowBlank: false,
            anchor: '90%'
        });
        this.dateRangeField = new iBOSe.view.reminder.src.form.field.DateRange({
            fieldLabel: 'When',
            singleLine: false,
            anchor: '90%'
        });
        this.reminderField = new iBOSe.view.reminder.src.form.field.ReminderCombo({
            name: 'Reminder',
            anchor: '70%'
        });
        this.notesField = new Ext.form.TextArea({
            fieldLabel: 'Notes',
            name: iBOSe.view.reminder.src.data.EventMappings.Notes.name,
            grow: true,
            growMax: 150,
            anchor: '100%'
        });
        this.locationField = new Ext.form.Text({
            fieldLabel: 'Location',
            name: iBOSe.view.reminder.src.data.EventMappings.Location.name,
            anchor: '100%'
        });
        this.urlField = new Ext.form.Text({
            fieldLabel: 'Web Link',
            name: iBOSe.view.reminder.src.data.EventMappings.Url.name,
            anchor: '100%'
        });
        ...
        if (this.calendarStore) {
            this.calendarField = new iBOSe.view.reminder.src.form.field.CalendarCombo({
                store: this.calendarStore,
                anchor: '70%',
                name: iBOSe.view.reminder.src.data.EventMappings.CalendarId.name
            });
    ...
    loadRecord: function(rec){
        ...
        if(this.calendarStore){
            this.form.setValues({
                'calendar': rec.data[iBOSe.view.reminder.src.data.EventMappings.CalendarId.name]
            });
        }
