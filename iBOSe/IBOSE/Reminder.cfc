...
<cfif rem NEQ "" > <!--- The reminder is set to None. None means no reminder. The event saved is treated as a note. --->
	...
	<cfquery name="getProfileName" datasource="#session.global_dsn#" maxrows="1">
		...
		<cfschedule
		    ...
		    url 			= "#session.domain#myapps/reminder/schedule/notify.cfm?ad=#urlencodedformat(ad)#&id=#urlencodedformat(id)#&cid=#urlencodedformat(cid)#&end=#urlencodedformat(end)#&loc=#urlencodedformat(loc)#&notes=#urlencodedformat(notes)#&rem=#urlencodedformat(rem)#&start=#urlencodedformat(startA)#&title=#urlencodedformat(title)#&urlstring=#urlencodedformat(urlstring)#&recipient=#urlencodedformat(getProfileName.PROFILENAME)#&pid=#urlencodedformat(session.chapa)#&dquery=#urlencodedformat(dquery)#&global_dsn=#urlencodedformat(session.global_dsn)#&company_dsn=#urlencodedformat(session.company_dsn)#&ms=#URLEncodedFormat(session.ms)#&mu=#URLEncodedFormat(session.mu)#&mp=#URLEncodedFormat(session.mp)#&mt=#session.mt#&mpt=#session.mpt#&mtls=#session.mtls#&mssl=#session.mssl#"
			interval		="once"
